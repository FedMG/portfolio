...
const PAGE_ALIGN_BREAKPOINT = 'px-6 sm:px-10 lg:px-16 xl:px-24 py-2.5 mx-16'
  ...
  return (
    <Header labelledby='header-title' className='bg-gray-100 sticky top-0 z-50'>
      <Navigation
        ...
        className={`border-b shadow-b-md border-gray-200 ${PAGE_ALIGN_BREAKPOINT}`}>
            ...
            {isValidArray(pages) &&
              pages.map(({ path, label }) => (
                <li
                  ...
                  <a
                    href={path}
                    className='py-3 px-7 sm:px-6 block text-gray-800 hover:text-[#764EF3] text-md font-medium border-b border-gray-100 lg:border-0'>
                    {label}
                  </a>
                </li>
              ))}
